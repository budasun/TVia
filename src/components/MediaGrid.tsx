...
function extractSeasonId(mediaId: string): string | null {
  const match = mediaId.toLowerCase().match(/^(breakingbad|simpsons|bettercallsaul|onepiece|blackmirror|bebereno|futurama|lotr|mandalorian|strangerthings)-t(\d+)$/);
  if (match) {
...
function extractSeasonFromEpisode(mediaId: string): string | null {
  ...
  let match = lowerId.match(/^(breakingbad|simpsons|futurama|lotr|strangerthings)-t?(\d+)x(\d+)$/);
  if (match) {
    const series = match[1];
    const seasonNum = parseInt(match[2], 10);
    return `${series}-t${seasonNum}`;
  }
  match = lowerId.match(/^(bettercallsaul|onepiece|blackmirror|mandalorian)-t(\d+)x(\d+)$/);
  if (match) {
    const series = match[1];
    const seasonNum = parseInt(match[2], 10);
    return `${series}-t${seasonNum}`;
  }
  match = lowerId.match(/^(bebereno)-t(\d+)x(\d+)$/);
  if (match) {
    const series = match[1];
    const seasonNum = parseInt(match[2], 10);
    return `${series}-t${seasonNum}`;
